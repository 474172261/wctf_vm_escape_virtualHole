...
mainState mega_main = {0};


void get_flag(void *dst)
{
    char flag[] = "Nice to meet you!";
    memcpy(dst, flag, sizeof(flag));
}

void megasas_easy_write(void *dst, void *src, uint32_t size){
    memcpy(dst, src, size);
}

int megasas_framebuffer_readback(mainState *mega_main, uint32_t block_id)
{
    frame_header *header = mega_main->frame_header;
    void *dst = header->frame_buff;
    ...
    header->write(dst, block->buffer + offset, size);
    return 0;
}
...
void handle_plus_write(mainState *mega_main,
                            uint32_t addr, uint32_t val)
{
    ...
    switch(addr){
        case 1:
            if(val > 0x80000)
                val = 0;
            mega_main->block_size = val;
            break;
        case 2:
            if(mega_main->block_size){
                if(val < MAX_BLOCK_ID){
                    if(!Blocks[val].buffer){
                        Blocks[val].buffer = calloc(mega_main->block_size, 1);
                        Blocks[val].size = mega_main->block_size;
                    }
                }
            }
            break;
        case 3:
            if(val < MAX_BLOCK_ID){
                if(Blocks[val].buffer){
                    free(Blocks[val].buffer);
                    Blocks[val].buffer = NULL;
                    Blocks[val].size = 0;
                }
            }
            break;
        case 4:
            if(!header && mega_main->data_size == 0x200){
                mega_main->frame_header = calloc(sizeof(frame_header), 1);
                mega_main->frame_header->get_flag = get_flag;
                mega_main->frame_header->write = megasas_easy_write;
                mega_main->frame_header->frame_buff = calloc(mega_main->data_size, 1);
                mega_main->frame_header->size = mega_main->data_size;
            }
            break;
        case 5:
            if(header){
                if(header->frame_buff){
                    free(header->frame_buff);
                }
                free(header);
                mega_main->frame_header = NULL;
            }
            break;
        case 6:
            if(val){
                mega_main->data_size = val;
            }
            break;
        case 7:
            if(header){
            ...
            break;
        case 8:
            if(header && header->size == 0x200){
                if(header->size >= mega_main->data_size){
                    pci_dma_read(mega_main->pci_dev, val,
                        header->frame_buff,
                        mega_main->data_size);
                }
            }
        ...
        case 9:
            if(header && header->size == 0x200){
                if(header->size >= mega_main->data_size){
                    pci_dma_write(mega_main->pci_dev, val,
                        header->frame_buff,
                        mega_main->data_size);
                }
            }
        ...
        case 10:
            if(header)
                megasas_framebuffer_store(mega_main, val);
            break;
        case 11:
            if(header)
                megasas_framebuffer_readback(mega_main, val);
            break;
        case 12:
            megasas_quick_read(mega_main, val);
        default:
            break;
    }
}
